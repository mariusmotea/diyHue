...
uint8_t devicesPins[devicesCount] = {15};
uint8_t ledPin = 0;
uint8_t ledRelay = 2;

const uint8_t buttonPin = 13;    // the pin that the pushbutton is attached to
    ...
    for (uint8_t i = 0; i < server.args(); i++) {
      if (server.argName(i) == "light") {
        ...
        if (server.arg(i) == "True" || server.arg(i) == "true") {
          ...
          digitalWrite(ledRelay, LOW); // inverse
        }
        else {
          ...
          digitalWrite(ledRelay, HIGH); // inverse
        }
      }
    }
...
void loop() {
  ...
  if (buttonState == LOW) { // Blitzwolf has button reversed
    for (uint8_t device = 0; device < devicesCount; device++) {
